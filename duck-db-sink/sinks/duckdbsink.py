...
class DuckDBSink(BatchingSink):  # Changed from BaseSink to BatchingSink
    ...
    def write(self, batch: SinkBatch):
        ...
        try:
            conn.execute("BEGIN TRANSACTION")  # Start the transaction
            ...
            for write_batch in batch.iter_chunks(n=self.batch_size):
                ...
                try:
                    ...
                    logger.error(f"Error writing to DuckDB due to error: {e}")
                    # An error occurred, raise a SinkBackpressureError
            ...
            conn.execute("COMMIT")  # Commit the transaction if everything goes well

        except duckdb.Error as exc:
            logger.error("Transaction failed, rolling back. Error: %s", exc)
            conn.execute("ROLLBACK")  # Rollback in case of failure
            raise
        finally:
            conn.close()